...
const createTokenAndMintTo = async (
  ...
  let mintToIxs = mintTo.flatMap(({ recepient, amount }) => {
    ...
    return [
      ...
      createMintToInstruction(tokenMint, ataAddress, mintAuthority, amount, [], TOKEN_PROGRAM),
    ];
  });
...
describe('escrow', () => {
  ...
  const [aliceUsdcAccount, aliceWifAccount, bobUsdcAccount, bobWifAccount] = [alice, bob].flatMap(
    (owner) =>
      [usdcMint, wifMint].map((tokenMint) =>
        getAssociatedTokenAddressSync(tokenMint.publicKey, owner.publicKey, false, TOKEN_PROGRAM)
      )
  );
  ...
  test('Offer taken by Bob, tokens balances are updated', async () => {
    ...
    const [offerAddress, _offerBump] = PublicKey.findProgramAddressSync(
      [Buffer.from('offer'), alice.publicKey.toBuffer(), offerId.toArrayLike(Buffer, 'le', 8)],
      program.programId
    );
